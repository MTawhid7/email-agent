...
# ── Social platform SVG icon paths ────────────────────────────────────────────
# Keyed by lowercase label. Values are the SVG path `d` attribute(s) plus fill rule.
# All icons are sized to a 24×24 viewBox and rendered at 18×18px.

_ICON_SVG: dict[str, str] = {
    "linkedin": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#0A66C2" style="vertical-align:middle;">'
    ...
    "whatsapp": (
        ...
        '0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c'
        '.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 '
    ...
    "instagram": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#E4405F" style="vertical-align:middle;">'
        ...
        '2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 '
        '4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 '
        '4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-'
        '4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 '
        '0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a'
        '1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>'
        '</svg>'
    ),
    "twitter": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#000000" style="vertical-align:middle;">'
        '<path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.76'
    ...
    "facebook": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#1877F2" style="vertical-align:middle;">'
        ...
        '2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385'
        'C19.612 23.027 24 18.062 24 12.073z"/>'
        '</svg>'
    ),
    "github": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#24292F" style="vertical-align:middle;">'
    ...
    "youtube": (
        '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
        'fill="#FF0000" style="vertical-align:middle;">'
        '<path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-'
        '7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 '
        '3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 '
        '002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-'
        '6.273 3.568z"/>'
        '</svg>'
    ),
}

# Fallback: globe icon for any unrecognised label
_GLOBE_SVG = (
    '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" '
...
def _icon_for(label: str) -> str:
    """Return the inline SVG for a known platform label, or the globe fallback."""
    return _ICON_SVG.get(label.lower().strip(), _GLOBE_SVG)
...
class SignatureBuilder:
    ...
    def build_html(self) -> str:
        ...
        if s.social_links:
            icon_links = [
                f'<a href="{link["url"]}" title="{link["label"]}" '
                f'style="text-decoration:none;margin-right:6px;" '
                f'target="_blank" rel="noopener noreferrer">'
                f'{_icon_for(link["label"])}'
                f'</a>'
                for link in s.social_links
            ]
            lines.append("".join(icon_links))

        inner = "<br>\n".join(lines)
        return f"\n<br><hr>\n<p>\n{inner}\n</p>"

    def build_plain_text(self) -> str:
        """Plain-text fallback — icons can't be represented, so labels are used."""
        s = self._s
        lines: list[str] = ["--"]
